...
class Camera:
    ...
    def compare_squares(self, board, turn):
        ...
        return ssim_squares, detailed_squares
    ...
    def save_state_squares(self, squares):
        """Save the current squares to a folder."""

        """Update the previous turn squares with the current move squares."""
        if os.path.exists("previous_move"):
            os.rmdir("previous_move")
        os.rename("current_move", "previous_turn")

        current_squares = divide_into_squares(frame)
        save_squares(current_squares, "current_move")
        
        os.mkdir("previous_turn")
    ...
    def recognize_move(self, board, turn):
        ...
        og_squares, changed_squares_with_details = self.compare_squares(board, turn)
        ...
        if board.has_castling_rights(turn):
            ...
            if all(square_scores.get(sq, 0) > 25 for sq in kingside_squares) and kingside_move in board.legal_moves:
                self.save_state_squares(og_squares)
